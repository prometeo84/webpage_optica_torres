...
(function(){
  ...
  function parseVideo(src){
    ...
    try{
      ...
      if(/dailymotion/.test(u.hostname)) return {provider:'dailymotion', id: (u.pathname.split('/').pop()||'')};
  ...
  function thumbnailFor(info){
    if(!info) return '';
    if(info.provider==='youtube') return 'https://img.youtube.com/vi/'+info.id+'/hqdefault.jpg';
    if(info.provider==='dailymotion') return '/especialistas/gioconda-torres/thumb.php?id='+encodeURIComponent(info.id);
    return '';
  }

  function makePlaceholderFromIframe(iframe){
    var src = iframe.getAttribute('data-src') || iframe.src || iframe.getAttribute('src');
    ...
    wrapper.innerHTML = '<span class="vp-thumb" style="display:block;background-size:cover;background-position:center;width:100%;height:100%;"></span><span class="vp-play" aria-hidden="true">▶</span>';
    ...
    wrapper.addEventListener('click', function(){
      ...
      var srcUrl = '';
      if(info && info.provider==='youtube') srcUrl = 'https://www.youtube.com/embed/'+info.id+'?autoplay=1';
      if(info && info.provider==='dailymotion') srcUrl = 'https://www.dailymotion.com/embed/video/'+info.id+'?autoplay=1';
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try{ initPlaceholders(); }catch(e){}
    try{ setCurrentYear('.current-year'); }catch(e){}
  });
